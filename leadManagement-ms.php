...
$sortColumn = isset($_GET['sort']) ? $_GET['sort'] : 'lead_id_ms';
...
$allColumns = ['lead_id_ms', 'lead_name_ms', 'bedrooms', 'pickup', 'dropoff', 'lead_date', 'phone', 'email', 'details', 'booking_status', 'created_at', 'Source', 'AssignedTo'];
...
$query = "SELECT * FROM leads_ms WHERE 1=1";

// Add search term filtering
if ($searchTerm) {
    $query .= " AND (lead_id_ms LIKE '%$searchTerm%' OR lead_name_ms LIKE '%$searchTerm%' OR bedrooms LIKE '%$searchTerm%' OR pickup LIKE '%$searchTerm%' OR dropoff LIKE '%$searchTerm%' OR lead_date LIKE '%$searchTerm%' OR phone LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%' OR details LIKE '%$searchTerm%' OR booking_status LIKE '%$searchTerm%' OR AssignedTo LIKE '%$searchTerm%' OR Source LIKE '%$searchTerm%')";
}
...
$lastAssignedPersonQuery = "SELECT AssignedTo FROM leads_ms ORDER BY lead_id_ms DESC LIMIT 1";
...
$cycleCount = 0;
$leadsCycleQuery = "SELECT AssignedTo FROM leads_ms ORDER BY lead_id_ms DESC";
...
$totalLeadsQuery = "SELECT COUNT(*) AS total FROM leads_ms WHERE 1=1";

// Add search term filtering
if ($searchTerm) {
    $totalLeadsQuery .= " AND (lead_id_ms LIKE '%$searchTerm%' OR lead_name_ms LIKE '%$searchTerm%' OR bedrooms LIKE '%$searchTerm%' OR pickup LIKE '%$searchTerm%' OR dropoff LIKE '%$searchTerm%' OR lead_date LIKE '%$searchTerm%' OR phone LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%' OR details LIKE '%$searchTerm%' OR booking_status LIKE '%$searchTerm%' OR AssignedTo LIKE '%$searchTerm%' OR Source LIKE '%$searchTerm%')";
}
...
<head>
    ...
    <title>Moving Select Leads Dashboard</title>
    <!-- Bootstrap CSS -->
        ...
        <div class="row">
            ...
            <main class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2" id="Main-Heading">Moving Select Leads Dashboard</h1>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                <?php if (in_array('lead_id_ms', $visibleColumns)) : ?><th class="sortable" data-sort="lead_id_ms">Lead ID</th><?php endif; ?>
                                <?php if (in_array('lead_name_ms', $visibleColumns)) : ?><th class="sortable" data-sort="lead_name_ms">Name</th><?php endif; ?>
                        ...
                        <tbody>
                            <?php while ($row = $result->fetch_assoc()) : ?>
                                <tr>
                                    <?php if (in_array('lead_id_ms', $visibleColumns)) : ?><td><?php echo $row['lead_id_ms']; ?></td><?php endif; ?>
                                    <?php if (in_array('lead_name_ms', $visibleColumns)) : ?><td class="editable" data-field="lead_name_ms" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['lead_name_ms']) ?></td><?php endif; ?>
                                    <?php if (in_array('bedrooms', $visibleColumns)) : ?><td class="editable" data-field="bedrooms" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['bedrooms']) ?></td><?php endif; ?>
                                    <?php if (in_array('pickup', $visibleColumns)) : ?><td class="editable" data-field="pickup" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['pickup']) ?></td><?php endif; ?>
                                    <?php if (in_array('dropoff', $visibleColumns)) : ?><td class="editable" data-field="dropoff" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['dropoff']) ?></td><?php endif; ?>
                                    <?php if (in_array('lead_date', $visibleColumns)) : ?>
                                        <td class="editable-date" data-field="lead_date" data-id="<?= $row['lead_id_ms'] ?>">
                                            <input type="date" class="form-control" value="<?= htmlspecialchars($row['lead_date']) ?>" readonly>
                                        </td>
                                    <?php endif; ?>
                                    <?php if (in_array('phone', $visibleColumns)) : ?><td class="editable" data-field="phone" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['phone']) ?></td><?php endif; ?>
                                    <?php if (in_array('email', $visibleColumns)) : ?><td class="editable" data-field="email" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['email']) ?></td><?php endif; ?>
                                    <?php if (in_array('details', $visibleColumns)) : ?>
                                        <td class="editable" data-field="details" data-id="<?= $row['lead_id_ms'] ?>">
                                    ...
                                    <?php endif; ?>
                                    <?php if (in_array('booking_status', $visibleColumns)) : ?><td class="editable" data-field="booking_status" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['booking_status']) ?></td><?php endif; ?>
                                    <?php if (in_array('created_at', $visibleColumns)) : ?><td style="display: none;"><?php echo $row['created_at']; ?></td><?php endif; ?>
                                    <?php if (in_array('Source', $visibleColumns)) : ?><td class="editable" data-field="Source" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['Source']) ?></td><?php endif; ?>
                                    <?php if (in_array('AssignedTo', $visibleColumns)) : ?><td class="editable" data-field="AssignedTo" data-id="<?= $row['lead_id_ms'] ?>"><?= htmlspecialchars($row['AssignedTo']) ?></td><?php endif; ?>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
                    </table>
